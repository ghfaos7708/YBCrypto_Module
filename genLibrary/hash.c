...
EXIT: 
    if(ret != SUCCESS)  fprintf(stdout, "=*Location : YBCrypto_Hash_Init         =\n");
    parameter_flag = 0x00;
    state =  0x00;
    return ret;
}
...
EXIT: 
    if(ret != SUCCESS)  fprintf(stdout, "=*Location : YBCrypto_Hash_Update       =\n");
    parameter_flag = 0x00;
    state =  0x00;
    return ret;
}
...
EXIT: 
    if(ret != SUCCESS)  fprintf(stdout, "=*Location : YBCrypto_Hash_Final        =\n");
    parameter_flag = 0x00;
    state =  0x00;
    YBCrypto_memset(&HM, 0x00, sizeof(HashManager));
    return ret;
