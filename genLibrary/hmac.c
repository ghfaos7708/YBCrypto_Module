...
EXIT:
    if (ret != SUCCESS) fprintf(stdout, "=*Location : YBCrypto_HMAC_Init         =\n");
    parameter_flag = 0x00;
    state =  0x00;
    return ret;
}
...
EXIT:
    if (ret != SUCCESS) fprintf(stdout, "=*Location : YBCrypto_HMAC_Update       =\n");
    parameter_flag = 0x00;
    state =  0x00;
    return ret;
}
...
EXIT:
    if (ret != SUCCESS) fprintf(stdout, "=*Location : YBCrypto_HMAC_Final        =\n");
    parameter_flag = 0x00;
    state =  0x00;
    YBCrypto_memset(&MM, 0x00, sizeof(HashManager));
    return ret;
