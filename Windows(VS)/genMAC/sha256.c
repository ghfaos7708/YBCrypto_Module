...
#define ROTL_UINT(x, n) ((UINT)((x) << (n)) | (UINT)((x) >> (32 - (n))))
#define ROTR_UINT(x, n) ((UINT)((x) >> (n)) | (UINT)((x) << (32 - (n))))

#define ENDIAN_REVERSE_UINT(dwS)	( (ROTL_UINT((dwS),  8) & 0x00ff00ff)	\
									| (ROTL_UINT((dwS), 24) & 0xff00ff00) )

#define BIG_B2D(B, D)		D = ENDIAN_REVERSE_UINT(*(UINT_PTR)(B))
#define BIG_D2B(D, B)		*(UINT_PTR)(B) = ENDIAN_REVERSE_UINT(D)
#define LITTLE_B2D(B, D)	D = *(UINT_PTR)(B)
#define LITTLE_D2B(D, B)	*(UINT_PTR)(B) = (UINT)(D)
#define GetData(x)	ENDIAN_REVERSE_UINT(x)

#define RR(x, n)		ROTR_UINT(x, n)
#define SS(x, n)		(x >> n)
...
void SHA256_Transform(UINT_PTR Message, UINT_PTR ChainVar)
{
	UINT a, b, c, d, e, f, g, h, T1, X[64];
	UINT j;

	// 4 byte 단위로 big endian으로 변경함
	for (j = 0; j < 16; j++)
...
// o 출력				: 
//*********************************************************************************************************************************
void SHA256_Process(OUT SHA256_INFO* Info,
	IN const BYTE* pszMessage,
	IN UINT uDataLen)
{
	...
	{
		memcpy((UCHAR_PTR)Info->szBuffer, pszMessage,
			(SINT)SHA256_DIGEST_BLOCKLEN);
		SHA256_Transform((UINT_PTR)Info->szBuffer, Info->uChainVar);
...
void SHA256_Close(OUT SHA256_INFO* Info, IN BYTE* pszDigest)
{
	...
	Info->uLowLength = ENDIAN_REVERSE_UINT(Info->uLowLength);
	Info->uHighLength = ENDIAN_REVERSE_UINT(Info->uHighLength);

	((UINT_PTR)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 2] = Info->uHighLength;
	((UINT_PTR)Info->szBuffer)[SHA256_DIGEST_BLOCKLEN / 4 - 1] = Info->uLowLength;
